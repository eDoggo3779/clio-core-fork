...
class ClioCteBench(Application):
    ...
    def start(self):
        ...
        if self.config['nprocs'] > 1:
            ...
            # shared_dir; _get_stat globs + folds them. PsshExecInfo can't take
            # pipe_stdout, so the redirect is embedded via double-quoted bash -c
            # with an escaped \$(hostname) that expands on the REMOTE host.
            # (Same per-host capture mofka_bench uses for its 2n sweep.)
            self.log(
            ...
            cmd_str = f'bash -c "{cmd_str} > {output_path}.\\$(hostname) 2>&1"'
        else:
