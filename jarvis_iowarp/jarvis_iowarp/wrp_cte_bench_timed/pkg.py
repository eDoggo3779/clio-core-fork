...
class WrpCteBenchTimed(Application):
    """
    CTE Core Timed Benchmark Application

    Runs Put, Get, and PutGet benchmarks against a running CTE instance
    for a fixed duration (in seconds) and collects throughput and latency
    statistics.
    ...
    def _configure_menu(self):
        return [
            ...
            {
                'name': 'duration_sec',
                'msg': 'Duration to run each benchmark (seconds)',
                'type': int,
                'default': 30,
            },
            {
    ...
    def _configure(self, **kwargs):
        self.log("Configuring CTE timed benchmark application...")
        ...
        self.log("CTE timed benchmark configuration completed successfully")
    ...
    def start(self):
        ...
        result = Exec(cmd_str, exec_info).run()

        # Propagate subprocess exit codes. Jarvis' pipeline status is driven
        # by whether start() raises, so a failed benchmark must surface here.
        exit_codes = getattr(result, 'exit_code', {}) or {}
        nonzero = {h: c for h, c in exit_codes.items() if c != 0}
        if nonzero:
            self._log_output_tail(output_path)
            raise RuntimeError(
                f"Benchmark exited with non-zero code(s): {nonzero}")

        # Even on exit 0, require the output file to be present and carry
        # the expected header for the requested test case. Catches cases
        # where the child exits normally but wrote nothing, or the output
        # does not match what was asked for.
        self._check_output_freshness(output_path)
    ...
    def _check_output_freshness(self, path):
        ...
        if parsed_op != want:
            self._log_output_tail(path)
            raise RuntimeError(
                f"Benchmark header mismatch: output reports '{parsed_op}' "
                f"but test_case='{want}'")
